...
while ($reader->name === 'record' )
{
    ...
    if ( (!isset($resultarray['preferred_label_occur_2'])) && (isset($resultarray['refPagina'])) ) {
        //error -> wegschrijven
        $message = "WARNING: refPagina gegeven zonder preferred_label_occur_2 (documentation.title)";
        $log->logInfo($message);
        $fout[] = $resultarray;
        ...
        if (isset($resultarray['refNote'])) {
            $occur[$sleutel]['refNote'] = $resultarray['refNote'];
        }
        if (isset($resultarray['refAuteur'])) {
            $occur[$sleutel]['refAuteur'] = $resultarray['refAuteur'];
        }
    }
    $reader->next();
}
